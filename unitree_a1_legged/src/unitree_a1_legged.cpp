...
const UDP & UnitreeLegged::getLowUdp() const
{
  return low_udp_;
}
const LowCmd & UnitreeLegged::getLowCmd() const
{
  return low_cmd_;
}
const LowState & UnitreeLegged::getLowState() const
{
  return low_state_;
}
...
{
  std::for_each(
    std::begin(low_cmd_.motorCmd), std::end(low_cmd_.motorCmd), [mode](auto & motorCmd) {
      motorCmd.mode = mode;
    });
