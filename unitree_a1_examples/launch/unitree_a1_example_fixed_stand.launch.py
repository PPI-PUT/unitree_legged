...
def launch_setup(context, *args, **kwargs):
    param_path = LaunchConfiguration(
        'unitree_a1_example_fixed_stand_param_file').perform(context)
    if not param_path:
        param_path = PathJoinSubstitution(
            [FindPackageShare('unitree_a1_examples'), 'config',
             'unitree_a1_example_fixed_stand.param.yaml']
        ).perform(context)

    unitree_a1_examples_node = Node(
        package='unitree_a1_examples',
        executable='unitree_a1_examples_fixed_stand_node_exe',
        name='unitree_a1_example_fixed_stand',
        parameters=[
            param_path
        ],
        remappings=[
...
def generate_launch_description():
    ...
    add_launch_arg('unitree_a1_example_fixed_stand_param_file', '')
