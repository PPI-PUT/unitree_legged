...
def launch_setup(context, *args, **kwargs):
    param_path = LaunchConfiguration(
        'unitree_a1_example_torque_param_file').perform(context)
    if not param_path:
        param_path = PathJoinSubstitution(
            [FindPackageShare('unitree_a1_examples'), 'config',
             'unitree_a1_example_torque.param.yaml']
        ).perform(context)

    unitree_a1_examples_node = Node(
        package='unitree_a1_examples',
        executable='unitree_a1_examples_torque_control_exe',
        name='unitree_a1_example_torque',
        parameters=[
            param_path
        ],
        remappings=[
            ...
            ("~/output/torque", LaunchConfiguration("output_torque_name"))
        ],
        output='screen',
